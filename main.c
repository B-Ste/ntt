...
void nwc_naive(int64_t, int64_t*, int64_t*, int64_t*);
void nwc_ntt(int64_t, int64_t*, int64_t*, int64_t, int64_t*, int64_t*, int64_t*);
void ntt(int64_t, int64_t*, int64_t*);
void intt(int64_t, int64_t, int64_t*, int64_t*);
int64_t* brv_powers(int64_t, int64_t, int64_t);
int64_t brv(int64_t, int64_t);

int64_t mod(int64_t a, int64_t q) {
    int64_t r = a % q;
    return r < 0 ? r + q : r;
}
...
int main(int argc, char const *argv[]) {
    ...
    int64_t* c1 = malloc(N * sizeof(int64_t));
    int64_t* c2 = malloc(N * sizeof(int64_t));
    int64_t q = modulus[k];
    int64_t psi_p = psi[k];
 ...
 */
void nwc_naive(int64_t q, int64_t* a, int64_t* b, int64_t* c) {
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            int64_t c_k = mod(a[i] * b[j], q);
 ...
 * and saves it in c.
 */
void nwc_ntt(int64_t q, int64_t* psis, int64_t* psi_ns, int64_t n_neg, int64_t* a, int64_t* b, int64_t* c) {
    ntt(q, psis, a);
    ntt(q, psis, b);
    for (int i = 0; i < N; i++) c[i] = mod(a[i] * b[i], q);
 ...
 */
void ntt(int64_t q, int64_t* psis, int64_t* a) {
    int64_t t = N;
    for (int m = 1; m < N; m *= 2) {
        t /= 2;
        for (int i = 0; i < m; i++) {
 ...
 * in normal order.
 */
void intt(int64_t inv_n, int64_t q, int64_t* psis, int64_t* a) {
    int64_t t = 1;
    for (int m = N; m > 1; m /= 2) {
        int64_t j_1 = 0;
        int64_t h = m / 2;
